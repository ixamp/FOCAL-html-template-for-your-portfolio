...
let path = {
  ...
  watch: {
    ...
    img: source_folder + '/img/src/**/*{.JPG,.jpg,.PNG,.png,.gif,.GIF,.svg,.SVG,.webp,.WEBP,.ico,.ICO}'
  },
...
function copyFiles() {
  src(source_folder + '/copyfiles/js/**/*')
    .pipe(dest(project_folder + '/js/'))
  return src(source_folder + '/copyfiles/css/**/*')
...
let build = gulp.series(clean, gulp.parallel(css, html, js, images, fonts, copyFiles), fontsStyle);
