...
jQuery(function ($) {
  ...
  if ($('.slider-top').length) {
    ...
    $('ul.slick-dots li').empty(); // for (let i = 0; i < $('.slider-top .slide-container [data-ixamp-type]').length; i++) {
    // }
  ...
  if ($('.percents-line').length) {
    $(window).scroll(function () {
      var allHeightWindow = $(window).scrollTop();
      var blockStatus = true;

      for (var i = 0; i < $('.progress-bars').length; i++) {
        if (allHeightWindow >= $($('.progress-bars')[i]).offset().top - 400 && blockStatus) {
          ...
          for (var j = 0; j < progressBar.length; j++) {
            ...
            $(progressBar[j]).animate({
              num: endGrow - startGrow
            }, {
              duration: speedGrow * 1000,
              step: function step(val, params) {
